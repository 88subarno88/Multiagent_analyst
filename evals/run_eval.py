...
async def run_eval(strategy: str, provider: str, out_name: str):
    ...
    # Fixed yardstick judge (always Gemini so scores stay comparable across runs).
    judge = LLMClient(provider="ollama")

    sem = asyncio.Semaphore(settings.eval_concurrency)

    async def _eval_one(idx: int, row: dict) -> dict:
        async with sem:
            ...
            try:
                ...
                context = _flatten_context(res.worker_results)
                cost_usd = res.cost.get("cost_usd", 0.0)
                total_tokens = res.cost.get("total_tokens", 0)
    ...
    results = await asyncio.gather(*(_eval_one(i, row) for i, row in enumerate(dataset)))
    ...
    final_output = {
        "config": {"strategy": strategy, "provider": provider},
        "aggregate": agg,
        "results": results,
    }
